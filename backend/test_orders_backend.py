...
from backend.model_mongodb import Order
import requests
import ast
import pytest
...
def test_find_by_status():
    expected = [monky]
    actual = Order({}).find_by_status("in transit")
    assert actual == expected

def test_find_by_quantity():
    expected = [monky]
    actual = Order({}).find_by_quantity("2")
    assert actual == expected

def test_find_by_price():
    expected = [monky]
    actual = Order({}).find_by_price("100000000000000")
    assert actual == expected

def test_find_by_productID():
    expected = [monky]
    actual = Order({}).find_by_productID("42")
    assert actual == expected
